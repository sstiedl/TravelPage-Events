...
class Locations {
    ...
    constructor(locName: string, locAdress: string, image: string, created: any){
...
class Restaurants extends Locations{
        ...
        constructor(locName: string, locAdress: string, image: string, cuisineGenre: string, website: string, phoneNumber: string, created: any){
...
class Events extends Restaurants {
    ...
    constructor(locName: string, locAdress: string, image: string, cuisineGenre: string, website: string, phoneNumber: string, date: string, price: number, created: any){
...
let rests: Array<Restaurants> =[rest1, rest2];
...
let events: Array<Events> =  [event1, event2, event3];

for(let val of events){
    val.created = new Date(val.created);
}

let allLocations: Array<Locations> =[place1, place2, place3, place4, rest1, rest2, event1, event2, event3]
...
//Creating Layout
const createLayout = () =>{
    (document.getElementById("myContainer") as HTMLElement).innerHTML = "";
    (document.getElementById("myContainer") as HTMLElement).innerHTML += `<div class="row  m-0 mt-2 p-0 " id="locRow"><div class="fs-5 text-light myBg p-2">Places</div>`
    for(let val of places){
        
        (document.getElementById("locRow") as HTMLElement).innerHTML += val.display();
        
        }
        
    (document.getElementById("myContainer") as HTMLElement).innerHTML += `<div class="row m-0 mt-2 p-0 " id="restRow"><div class="fs-5 text-light myBg p-2">Restaurants</div>`
    for(let val of rests){
           
        (document.getElementById("restRow") as HTMLElement).innerHTML += val.display();
            
        }
    (document.getElementById("myContainer") as HTMLElement).innerHTML += `<div class="row m-0 mt-2 p-0  " id="evRow"><div class="fs-5 text-light myBg p-2">Events</div>`
     for(let val of events){
            
        (document.getElementById("evRow") as HTMLElement).innerHTML += val.display();
...
if(sorting == false){
    ...
    /* let dateA:any = new Date(a.created);
    let dateB: any = new Date(b.created) */
    return +dateB - +dateA
  });
  sorting = true;
  (document.getElementById("date-btn")as HTMLElement).innerHTML = `Date <b>&#10515;</b>&#8593;`;
  (document.getElementById("myContainer") as HTMLElement).innerHTML += `<div class="row m-0 mt-2 p-0" id="myRow"></div>`
  for(let val of allLocations){
      (document.getElementById("myRow") as HTMLElement).innerHTML += val.display();
...
(document.getElementById("cat-btn")as HTMLElement).addEventListener("click", createLayout);
(document.getElementById("date-btn")as HTMLElement).addEventListener("click", sortOnClick);
